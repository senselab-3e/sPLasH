...
var Config = {
    ...
    NUM_WORMS:            40,
    MIN_LENGTH:           5,
    MAX_LENGTH:           45, //for some reason the length also effects the thickness. >> this is because later in the code it does: length = random( Config.MIN_LENGTH, Config.MAX_LENGTH );                thickness = length * random( 0.5, 4.5 ); >> by changing the multiplier '4.5' to 0.5, it will make the thickness less. but  as is, the 'max length' is still subject to this multiplier, rather then acting as a ceiling. 
	MIN_THICKNESS:        5,
    MAX_THICKNESS:        15,
    MIN_SEGMENT_SPACING:  1,
    MAX_SEGMENT_SPACING:  10  //this will extend the length of the lines --------<<<
};


//import code from other canvas that will never work : P
    ...
    var ctx = Sketch.create({
        ...
        setup: function() {
            ...
            for ( var i = 0; i < Config.NUM_WORMS; i++ ) {
                ...
                thickness = length * random( 1.5, 5.5 );
                ...
                offset = new Vector( random( -100, 300 ), random( -100, 100 ) )
